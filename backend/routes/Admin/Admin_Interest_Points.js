...
router.post('/', (req, res) => {
  ...
  if (name == undefined
    ...
    const values = {
      type, name, SHAPE_Leng, SHAPE_Area, house_number, neighborhood, operator, handicapped, condition,
    };
...
router.get('/', (req, res) => {
  ...
  if (req.body.id) {
    ...
    db.query('SELECT * FROM places WHERE id = ? AND type != ? AND deleted = 0', [temp_id, globals.places_types.dog_park], (err, result) => {
      if (err) {
  ...
  // get all of the interest points
    ...
    db.query('SELECT * FROM places WHERE type != ? AND deleted =0', [globals.places_types.dog_park], (err, result) => {
      if (err) {
        globals.log_msg('GET /interest point - ERROR');
        console.error(err);
        res.status(globals.status_codes.Server_Error).json();
