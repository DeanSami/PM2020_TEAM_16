    ...
    if (err) {
      ...
      db.query('SELECT * FROM games WHERE id = ?', [req.query.game_id], (err, games) => {
        ...
        if (games.length == 0) {
          ...
          db.query('SELECT * FROM game_steps WHERE game_id = ? AND start_location = ? AND step_num = 1', [req.query.game_id, games[0].start_location], (err, game_step) => {
            if (err) {
              globals.log_msg('GET /user/games/startgame/:id - ERROR');
              console.error(err);
              res.status(globals.status_codes.Server_Error).json();
...
router.post('/create', (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE id = ? AND user_type = ?', [req.body.owner_id, 2], (err, result) => {
...
router.patch('/edit', (req, res) => {
  ...
  /* check for existing owner user */
  db.query('SELECT * FROM users WHERE id = ? AND user_type = ?', [req.body.owner_id, globals.user_types.businessOwner], (err, result) => {
    if (err || !result || !result.length) {
      globals.log_msg('POST /games/create - ERROR in search owner');
      console.error(err);
      res.status(globals.status_codes.Server_Error).json({ message: 'can not find owner by owner_id' });
